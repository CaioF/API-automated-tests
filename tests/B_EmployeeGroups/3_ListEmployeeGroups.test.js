...
// GET /api/v1/ListEmployeeGroups
describe('Returns a list of all Employee Groups with pagination', () => {
  ...
  it('returns 200 when the path params do match the swagger specification', (done) => {
    ...
    api()
    .expectStatus(200)
    .expectValue('employeeGroups[0].ID', 2)
    .end( (err, res, body) =>
    {  
      if (err) {
        throw new Error(`\nMOCHA ERR:\n${err.message}\n\nRESPONSE ERR:\n${JSON.stringify(body)}`)
  ...
  it('returns 200 when the path params are omitted', (done) => {
    ...
    .expectStatus(200)
    .expectValue('employeeGroups[0].ID', 1)
    .end( (err, res, body) =>
    {  
      if (err) {
        throw new Error(`\nMOCHA ERR:\n${err.message}\n\nRESPONSE ERR:\n${JSON.stringify(body)}`)
